...
> 物流执行子集原有 15 项、首页八项；财务知识库现有 **55 项**定义，登记数不等于任意财报可算数。
...
| **U1** | 主题、指标可用性与比较合同 | ✅ done（`8eedd93`） |
| **U2** | 确定性拆解、联动提示与边界测试（五原语 + 腾讯映射 + 杜邦口径标注） | ✅ 实质完成（`861acad`；剩 2 项外部依赖） |
| **U3** | 同源证据与快照分析投影 | 🚧 当前断点 |
| U4 | 独立全行验证与留出泛化（依赖 oracle 人工录入） | ⬜ |
| U5–U7 | 客观报告门禁、统一冻结与真实 PDF、四问工作台 | ⬜ |
| U8–U10 | 运行保障、内部试点、证据决策 | ⬜ |
...
| 指标引擎 | 物流 15 指标 + 知识库 55 指标定义、Decimal、月/YTD/预算/同比/T12 窗口 | 登记数不等于任意财报可算数 |
...
| 财务知识库（客观分析主线） | 55 指标、167 科目、48 准则登记、32 分录模板 | 库内版本化文档（D048），YAML 兜底 |
