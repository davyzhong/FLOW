...
export function MarginMatrix({ matrix }: { matrix: DashboardResponse["margin_matrix"] }) {
  ...
  return (
    <section className="panel matrix-panel">
      ...
      <div className="table-scroll" role="region" aria-label="毛利矩阵横向滚动区域" tabIndex={0}><table aria-label="客户群与产品毛利矩阵"><thead><tr><th>客户群</th>{matrix.columns.map((column) => <th key={column.id}>{column.name}</th>)}</tr></thead>
