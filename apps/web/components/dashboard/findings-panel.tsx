  ...
  return (
    <section className="panel findings-panel" aria-label="重点经营发现">
      ...
      <ol>{findings.map((finding, index) => <li key={finding.finding_id}>
        ...
        <div><h4><a className="finding-title-link" href={finding.investigation_path}>{finding.title}</a></h4><p>影响 <strong className={`is-${finding.impact.semantic_direction}`}>{finding.impact.display_value}</strong> · 证据 {finding.evidence_verified}/{finding.evidence_total} · 得分 {finding.total_score}</p></div>
