...
M1 必须建立 `docs/10_governance/schemas/document-metadata.schema.json`，以 JSON Schema `oneOf` 按 `doc_type` 校验必填字段和状态，并在 CI 中校验 M1 后新增或修改的正式文档。历史文档在 M0 登记为 `legacy-exempt`，无需一次性补齐元数据；一旦被实质修订或提升为当前文档，就必须通过 schema。

M2 另建立 `docs/knowledge-base/00_governance/knowledge-artifact.schema.json`，复用上述核心字段，并分别约束 knowledge-card、domain-handbook、taxonomy 和 product-mapping 的专用字段。知识发布 CI 覆盖所有拟进入 release lock 的资产；任一资产 schema 不合格，整个发布失败。

Schema 只验证字段形状。M1 同时建立跨文档完整性检查器并加入 CI，至少执行：doc/knowledge/asset ID 唯一；引用目标存在；上游 decision 为 accepted、spec 为 approved、knowledge release 可解析；release-lock 路径和 SHA-256 一致；相对目标分支的状态转换合法；canonical 知识具有来源；工作包不得绕过 approved spec。任一失败均阻止文档发布、知识发布或计划状态前移。
...
| 批次 | 目标 | 主要产物 | 不做什么 |
|---|---|---|---|
| M0 | 建立全量清单 | 文档类型、状态、权威、接替、机器引用登记 | 不移动文件 |
| M1 | 建立稳定骨架 | 新 README、起点、治理规则和路径映射 | 不改产品合同 |
| M2 | 发布静态知识 v1 | 来源登记、知识卡、领域手册、产品映射 | 不动态追读 Obsidian |
...
- 首版 `flow-knowledge-2026-09-12.1` 的 `coverage.tsv` 恰有 3035 条唯一来源记录，并通过 source ID 唯一性、总数、截面 ID 和基线哈希检查；后续版本通过继承哈希与 delta 哈希检查；
...
2. 当前已实现、正在执行、被阻塞的工作分别从哪里读取？
3. 当前唯一计划在哪里，是否存在第二份可执行总计划？
4. 某项财务公式、业务方法和产品功能分别以什么文档为权威？
5. 如何从一条产品规格追溯到知识版本和原始来源？
6. Obsidian 不可访问时是否可以继续工作，允许何时刷新？
7. 哪些目录不可移动或重写？
8. 历史计划、已关闭审查和当前缺陷如何区分？
9. 内部材料怎样才能进入试点或产品？
10. 如何判断一个任务已经完成？

独立 Agent 最多读取 `READING_ORDER.md` 指定的 5 份起点文档作答。每题在测试前提交版本化评分 rubric，列出必答事实、规范来源、允许表述和关键错误条件；当前状态变化时先更新期望答案与 rubric 版本，再开始盲测。通过标准为 10 题至少 9 题完全满足 rubric，且第 2、3、4、6、7、9 题不得命中任何关键错误；同时必须正确区分研究候选、canonical 知识、正式决策、规格、计划和交付证据。题目、rubric、期望答案、实际回答和评分作为 M6 验证记录提交。
