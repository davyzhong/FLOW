...
> 本节数字为 2026-09-15 历史快照。2026-09-25 订正台账记录答案集覆盖为
> 1794/1794（100%）；本轮没有重跑基准，因此此数字仅按台账引用，不能据此宣称
> C 级出口通过。独立盲评与 holdout 抽签仍是当前外部门槛，现状见 CURRENT_ROADMAP。
...
## 3.7 出口裁决参数（2026-09-15 用户苏格拉底问答确认）

| 参数 | 裁决 |
|---|---|
| 出口主张 | 数字必须可信（质量唯一硬门槛；Q1） |
| holdout 结构 | 两层：回归基准层 + 新批次泛化层（Q2） |
| 回归层样本 | 指定最难一家 = 菜鸟招股书（压力测试定位；L1 弱锚集中，失败需归因区分「管线退步」vs「源文件固难」；Q3） |
| 准确率合格线 | 全量 100%，零容忍（Q4） |
| 分母定义 | 已定位值 100% + 未定位值人工翻页补齐（不得静默剔除；Q5） |
| 盲评方式 | 纯 AI 交叉评（不同厂商模型，评审 prompt 不泄漏答案集；两次确认，无人工兜底——独立性声明如实打折，出示外部前可升级；Q6/Q7） |
| 新批次抽签 | 预注册随机规则：财报到料当日由用户随机指定 1 家封存为 holdout，抽取完成才启封对账（Q8 用户重答正式确认，2026-09-15） |
