...
> **安全前置**：当前 `services/api/tests/conftest.py` 将未显式设置的 `DATABASE_URL`
> 指向常驻 `flow` 库；已发生测试误清共享数据。修好并验证测试库隔离/误连 fail-fast 前，
> 禁止在本地运行会清理或迁移数据库的测试命令；不得把补种当成根因修复。

## 顺序与状态（2026-09-25 对 main `58c5a5d` 核对；最新 CI 尚在运行）
...
0. **修复测试数据库隔离（安全阻塞项）**：取消本地测试默认连接常驻 `flow` 库；为破坏性/迁移测试提供显式、可识别的 disposable DB，目标库不符合隔离标记时 fail-fast；验证测试不会连接或清理开发库。完成前禁止本地执行 DB 清理/迁移型测试。
1. **C 级交叉评归因与原件复核**（上表3）：对报告列出的42个确认异常逐项回到原始PDF及字段合同，区分抽取器错误、文本层错误、证据包转换错误；裁决109个口径疑点；为JDL修复/替换不可读文本层并重核200格。只在归因后修订新版本订正记录，保留原值与证据。
2. **U4 解析器修复与真正的新留出**（上表1'）：支持「合并及公司」标题与年报页码提示区间参数化；旧三样本仅作为回归集。抽签候选见 [记录](../../validation/financial_reports/holdout-lottery-2026-09-25.md)，但仍须下载原件、冻结SHA、登记manifest、由独立会话录入oracle，首跑前禁止适配/调参。
3. **重新跑质量基准并完成 C 级 Go/No-Go**：依赖原件复核、必要修订、U4新留出独立录入和盲测；重跑L1并归档机器结果。交叉评与抽签均不单独构成通过，`1794/1794` 只表示已登记覆盖口径。
4. **大麦数据后的剩余 UX 收口**（上表2b）：G2现已完成；补齐精确页面/文件、五态矩阵、自动化与视觉验收规格，再执行导出审计、真实来源跳转、经分专员首页、固定下钻、报告渐进披露与治理折叠；不得重做前端Task 0–9。
5. **P3 数据与溯源接入**（上表3b）：修复/替换ZTO文本层，完成抽取、勾稽、页锚及API/UI链路测试；评审并实现真实来源链接；复核阿里分部序列在main的来源和可见性。
6. **知识 release 收尾**（上表K）：完成K0–K6，核对M2用户追认、`CURRENT_RELEASE`、sealed candidate/激活记录、Task 10三仓库证据和独立复验；用户裁决前不切指针。
7. **外部阻塞项**：取得《应用指南汇编2024》原文后核验rnd_exp；公开数据扩张C1等待真实财报。不得用合成样本替代。
8. **内部工作台与真实周期验证**（上表4/5，gated）：仅在C级Go及内部数据授权满足后启动；连续3个真实月度周期对同输入人工基准、盲评与工时门槛验收；U10按证据包裁决，不由模拟数据单独解锁。
