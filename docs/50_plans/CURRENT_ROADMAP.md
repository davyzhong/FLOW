...
| 2b | [大麦数据后的剩余体验收口](work_items/UX--post-damai-experience-closeout.md) | **active（Gate 1 诊断完成，代码验收待完成）** | G2 ✓；用户于 2026-09-26 批准实施 | 初始只读 API 响应矩阵与哈希已固定（基线 `3ff95115` + overlay `1ede2648…`，见[证据记录](../60_delivery/verification/2026-09-26--damai-visibility-gate1-v1.md)）；FY2025工作台500根因已修复并有7/7 API测试，但需干净SHA复测；其余缺口待逐项归因。 |
...
5. **大麦数据可见性与剩余 UX 收口**（上表2b，用户已批准）：Gate 1 的只读 API 诊断矩阵、请求口径和初始响应哈希已固定；快照是 `main@3ff95115` 加工作树 overlay `1ede2648…`，不是干净提交版本。FY2025 四问 500 的响应模型缺 `metric_code` 根因已修复并有回归测试；其余缺口逐项归因待完成。待其他会话改动归属明确并提交后，用干净 SHA 复测关闭 Gate 1，再进入 Gate 2–5。适用事实补齐、快照重建和页面验证只在隔离环境实施；不得重做前端 Task 0–9，不对常驻库执行未批准写入。
