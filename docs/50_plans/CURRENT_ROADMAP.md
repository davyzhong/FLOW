...
> 2026-09-26 更新：维度展示已按当前快照事实过滤，筛选器保留完整主数据目录，产品表/毛利矩阵显示覆盖分子/分母及无事实空态；提交 `649a2aba` 隔离 verify19/19、E2E9/9通过。注意：根 checkout 的旧 `make test-dashboard` 曾向常驻 `flow` 附加测试批次并改变 latest 选择（详情见工作包/证据记录）；没有删除或回滚，常驻库恢复待用户授权。Gate1完整矩阵与剩余页面/下钻仍未关闭。
...
| 2b | [大麦数据后的剩余体验收口](work_items/UX--post-damai-experience-closeout.md) | **active（维度事实范围与覆盖空态已修；Gate 1全路由/视口/403/错误态、Gate 3/4其余下钻、Gate 5 CI仍待完整验收）** | G2 ✓；用户于 2026-09-26 批准实施 | `649a2aba` 隔离 verify19/19、E2E9/9通过；筛选器保留完整目录，表格/矩阵只展示快照确有事实的维度并显示覆盖分母。常驻 `flow` 另有一条意外测试批次成为 latest；未删除/回滚，待用户授权处置。详见工作包。 |
