...
| 2b | [大麦数据后的剩余体验收口](work_items/UX--post-damai-experience-closeout.md) | **active（Gate 1 干净SHA复验待完成；Gate 2 静态事实完成；Gate 3 部分完成）** | G2 ✓；用户于 2026-09-26 批准实施 | FY2025工作台500已修复；新发行财报每份51行、指标覆盖FY2025 37/40、FY2026 40/40。静态 canonical 财务实际768条，OCF进入快照；隔离 seed/verify 19/19、E2E 9/9，趋势12/12可用。毛利矩阵根因：实际事实覆盖10/32、同比8/32，缺格不补零；比较选择按已发布覆盖数选择单一口径、预算优先并列。常驻库未写入。见[工作包](work_items/UX--post-damai-experience-closeout.md)与[证据记录](../60_delivery/verification/2026-09-26--damai-visibility-gate1-v1.md)。 |
...
5. **大麦数据可见性与剩余 UX 收口**（上表2b，用户已批准）：Gate 1 初始矩阵绑定 `main@3ff95115` + overlay `1ede2648…`，不是干净提交版。FY2025工作台契约故障已修复；Gate 2静态事实覆盖FY2025 37/40、FY2026 40/40；OCF已进入静态事实并在隔离栈验收（verify 19/19、E2E 9/9、趋势12/12可用）。毛利矩阵根因已定位：源实际10/32、同比8/32；缺组合不补零。当前服务端要求32格全有比较才选择口径，导致可用的部分预算比较被标作“不可用”。下一项按覆盖格数选单一比较类型，预算与同比覆盖相同则预算优先；缺格保持 unavailable，矩阵整体 degraded。随后完成隔离 API/UI 验证、Gate 1干净SHA复测、Gate 4页面/下钻与Gate 5全链回归。不得重做前端Task 0–9，不对常驻库执行未批准写入。
