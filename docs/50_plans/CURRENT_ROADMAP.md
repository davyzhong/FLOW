...
| 2a | [大麦物流完整财年演示数据](work_items/DAMAI--full-year-demo.md) | **active** | 已批准规格；不依赖公开 C 级 | D1–D3 实现已交付（A1–A4/B1–B3/C1/C2 全绿）：`DAMAI.SYN` 独立身份、正式审核链、原子幂等 seed、发行包零漂移、verify 19/19、八页面 E2E 9/9（`make damai-demo-up` / `make test-damai-demo-e2e`）；剩 C3 全量回归与 CI 关闭。合成数据只用于产品全链验证，不解除公开 C 级或真实企业门禁 |
| 2b | [大麦数据后的剩余体验收口](work_items/UX--post-damai-experience-closeout.md) | **blocked** | 大麦 D3 completed | 只承接导出审计、真实来源跳转、经分专员工作流首页、固定下钻、报告渐进披露和治理折叠；既有前端一致性 Task 0–9 不重做 |
