...
## 顺序与状态（2026-09-26 对 main `4ad34234` 核对；该 SHA 的 CI 状态须单独核验）
...
| 2b | [大麦数据后的剩余体验收口](work_items/UX--post-damai-experience-closeout.md) | **active（Gate 1 未关闭）** | G2 ✓；用户于 2026-09-26 批准实施 | 初查发现驾驶舱 OCF 趋势12/12缺值、KPI预算比较4/8缺值、毛利矩阵实际22/32及比较24/32缺值；FY2025四问 API 500。热重载工作树探测时变更未稳定，须同一已提交 SHA 复测后才进入 Gate 2。 |
...
5. **大麦数据可见性与剩余 UX 收口**（上表2b，用户已批准）：Gate 1 已形成初步读数；当前阻塞是 API 服务热重载工作树有未提交变更，盘点期间源状态改变，FY2025四问接口亦返回500。待变更提交或明确归属、运行环境稳定后，以一个已提交 SHA 重跑页面/API证据并归因，再进入 Gate 2–5。适用事实补齐、快照重建和页面验证只在隔离环境实施；不得重做前端 Task 0–9，不对常驻库执行未批准写入。
