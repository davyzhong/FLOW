...
> 2026-09-26 更新：维度展示已按当前快照事实过滤，筛选器保留完整主数据目录，产品表/毛利矩阵显示覆盖分子/分母及无事实空态；提交 `649a2aba` 隔离 verify19/19、E2E9/9通过，CI run `36232029817` success。状态记录提交 `7229cd0d`（run `36232204424`）与CI证据补记 `4c55f10e`（run `36232336053`）也均 success。注意：根 checkout 的旧 `make test-dashboard` 曾向常驻 `flow` 附加测试批次并改变 latest 选择（详情见工作包/证据记录）；没有删除或回滚，常驻库恢复待用户授权。Gate1完整矩阵与剩余页面/下钻仍未关闭。
