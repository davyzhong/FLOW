...
> 2026-09-26 更新：常驻开发库受控恢复已完成，verify19/19两次通过；dashboard API 200、8卡、趋势12/12、2 findings，仍degraded；静态覆盖37/40与40/40。经营分析点余额角色、派生指标依赖和比率格式已修复，主题缺口原因已分层；40次GET、36个不同路由/参数组合全部200；正式产物历史为空与数据库一致。Dashboard KPI未发布标签引发的20px视觉高度回归已在`31a60217`修正；该SHA CI仍排队，Gate1干净SHA视口/错误态/403/下钻矩阵未关闭。其他页面金额/数量格式仍待审计。隔离大麦全旅程verify19/19、E2E9/9。
...
| 2b | [大麦数据后的剩余体验收口](work_items/UX--post-damai-experience-closeout.md) | **active（Gate 1 API只读矩阵完成、clean-SHA/视口/403/错误态验收待完成；Gate 2静态事实完成；Gate 3部分完成；Gate 4批次历史clean-SHA通过、其余下钻待完成；Gate 5待最新提交SHA CI）** | G2 ✓；用户于 2026-09-26 批准实施 | 常驻库备份后幂等恢复，verify19/19两次，重复seed零增长。Dashboard API 200（8卡、趋势12/12、2 findings，degraded）；40次GET、36个不同路由/参数组合均200。经营分析点余额与派生公式依赖缺陷已修复，FY2026七项运营效率均可算；比率单位格式和公开披露不足/内部授权/期间缺口文案已区分。正式产物历史为空与数据库一致；其他页面金额/数量格式、全视口/错误态与下钻仍待审计。公式修复SHA隔离旅程verify19/19、E2E9/9。Gate1干净SHA矩阵与深链批次三待处理。见[工作包](work_items/UX--post-damai-experience-closeout.md)。 |
