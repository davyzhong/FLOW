...
| 2b | [大麦数据后的剩余体验收口](work_items/UX--post-damai-experience-closeout.md) | **active（Gate 1 干净SHA复验待完成；Gate 2 静态事实完成；Gate 3 部分完成）** | G2 ✓；用户于 2026-09-26 批准实施 | FY2025工作台500已在`48f8363`修复（7/7 API测试）；新发行财报每份51行，指标覆盖FY2025 37/40、FY2026 40/40。静态 canonical 财务实际已从672扩至768条，加入守恒的月度 OCF；canonical 25项、粒度对账3项和发行包检查通过。隔离 seed/verify 19/19通过，但UI E2E因共享Next `.next`锁失败，重跑前须给验收前端配置独立`distDir`。常驻库仍是旧事实；毛利矩阵 grain/comparison 缺失尚待 Gate 3 追查。见[证据记录](../60_delivery/verification/2026-09-26--damai-visibility-gate1-v1.md)。 |
