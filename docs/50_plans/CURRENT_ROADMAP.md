...
| 2b | [大麦数据后的剩余体验收口](work_items/UX--post-damai-experience-closeout.md) | **active（Gate 1 干净SHA复验待完成；Gate 2 静态事实完成；Gate 3 部分完成）** | G2 ✓；用户于 2026-09-26 批准实施 | FY2025工作台500已在`48f8363`修复（7/7 API测试）；新发行财报每份51行，指标覆盖FY2025 37/40、FY2026 40/40。静态 canonical 财务实际768条，月度 OCF 已进入快照；canonical 25项、粒度对账3项、发行包检查及隔离 seed/verify 19/19通过。浏览器 E2E 8/9通过，唯一失败是旧断言仍预期 OCF `unavailable`，实际为 `available`；重跑前先更正测试，并在临时 web 工作副本运行 Next，避免锁冲突和 tracked 配置改写。常驻库仍是旧事实；毛利矩阵 grain/comparison 缺失尚待 Gate 3 追查。见[证据记录](../60_delivery/verification/2026-09-26--damai-visibility-gate1-v1.md)。 |
...
5. **大麦数据可见性与剩余 UX 收口**（上表2b，用户已批准）：Gate 1 初始矩阵绑定 `main@3ff95115` + overlay `1ede2648…`，不是干净提交版。FY2025工作台契约故障已由`48f8363`修复；Gate 2 静态发行事实和指标覆盖已扩到 FY2025 37/40、FY2026 40/40（FY2025三项同比无FY2024比较期），相关24项测试通过。下一步需完成当前改动完整提交后的干净SHA复测，并在隔离栈装载、核验新报表；然后 Gate 3 修复快照发布/粒度缺失，Gate 4核页面/下钻，Gate 5全链回归。不得重做前端Task 0–9，不对常驻库执行未批准写入。
