...
| 2b | [大麦数据后的剩余体验收口](work_items/UX--post-damai-experience-closeout.md) | **active（Gate 1）** | G2 ✓；用户于 2026-09-26 批准实施 | 按 Gate 1 页面/API/事实缺口归因；本地只读 API 当前驾驶舱 `degraded`、指标覆盖 FY2025 22/40、FY2026 25/40；详见工作包 Gate 1–5；不重做前端 Task 0–9 |
...
5. **大麦数据可见性与剩余 UX 收口**（上表2b，用户已批准）：当前执行 Gate 1，只读建立逐页 API/事实/快照覆盖基线；完成并审查证据表后进入 Gate 2–5。适用事实补齐、快照重建和页面验证只在隔离环境实施；不得重做前端 Task 0–9，不对常驻库执行未批准写入。
