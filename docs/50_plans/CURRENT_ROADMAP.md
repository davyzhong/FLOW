...
| 2c | [企业组织建制与经营账套初始化包](work_items/ORG-LEDGER--enterprise-initialization-package.md) | **active（当前执行；组织 schema 待用户批准）** | 大麦发行包 completed；遵守安全规格 | 2026-09-27 用户批准：完整初始化同步组织并重建业务；业务重置保留组织；只作用于 manifest 企业。现有模型无人员/岗位档案，具体 schema 提案获批前不迁移。 |
...
截至 2026-09-27：**11 项，已完成 0 项，实际执行中 1 项，队首待启动/排队 9 项，外部材料受限 1 项**。本轮用户明确启动企业初始化包，当前执行为上表2c；其余按本唯一队列顺序执行。工作包中的 blocked/gated 表示出口/依赖尚未满足，不表示停止所有实施；状态表、工作包和历史文档是证据/规格，不构成第二份待办。

1. **[当前执行] 企业组织建制与经营账套初始化包**（工作包见状态表2c）：建立可移植双模块、合成组织档案、SQL 和一键全量/业务重置入口。先交付非破坏性包格式与校验；组织数据持久化需要具体 schema 提案和用户授权后才能迁移。所有破坏性验证仅限隔离栈。
