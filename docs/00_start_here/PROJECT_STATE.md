...
2026-09-26 更新：CI dashboard 数据库 URL 冲突已在 `scripts/test_dashboard.sh` 修复；脚本级安全回归 2/2、本机 dashboard Playwright 7/7通过，均将数据写入限制在 `flow_test`。修复提交后的 GitHub CI 待复验。此前关于两次 CI 失败的描述记录的是历史状态。常驻 `flow` 本轮未写入。
...
上段中的“两次 dashboard CI 失败”及“CI现需修正”仅记录本次修复前状态；以本页顶部更新为准：修复已完成，本提交 CI 待复验。
