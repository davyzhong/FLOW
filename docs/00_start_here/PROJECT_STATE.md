...
截至 2026-09-26，远端 `main` 当前基线为 `85efe850`，迁移头为 `0030_page_anchor_vocabulary`；该 SHA 的 CI 需按同一 SHA 单独核验。U8、S01、前端一致性 Task 0–9、大麦数据包和常驻库 G2 已完成；测试库隔离已于 `b60c51b` 修复，新的小米 2026H1 与阿里 FY2027Q1 holdout 已冻结并录入独立 oracle（`926af825`）。公开 C 级交叉评的42项确认异常已在 `4111f6a` 判定为真实抽取错误；109项口径疑点待裁决，京东物流200格英文原件已于 `b161fff` 冻结、待重核，抽取修订和新留出盲测未完成。大麦数据合同有24个月经营明细（1,920实际、10,752预算、4,800应收回款、672财务实际）；运行 API 初查显示两份年度报表、指标覆盖22/40与25/40。驾驶舱8张KPI卡，12个月趋势点（每点4指标），其中OCF趋势12/12缺失、KPI预算比较4/8缺失、毛利矩阵实际22/32与比较24/32缺失，整体 `degraded`；FY2025四问工作台500已定位为响应模型漏声明 `metric_code`，本地回归修复后返回200，待干净SHA复验。Gate 1 诊断响应矩阵首轮证据来自 `main@3ff95115` 加 overlay（SHA-256 `1ede2648…`），后续工作树已有进一步变更，不能将旧哈希当作当前实现指纹；其余缺口逐项归因仍待完成。后续主线只在 `main` 串行推进。
...
7. **大麦完整财年演示数据（底座 completed；页面覆盖未完成）**：24个月 synthetic 全链已落地，隔离栈 verify 19/19、八页面 E2E 9/9、重复 seed 零增长、发行包零漂移；常驻开发库 G2 已装载。只读 API 诊断快照：FY2025/FY2026各一份年度报表、每份40行覆盖四表；指标覆盖22/40、25/40；驾驶舱存在显著不可用比较值和OCF趋势缺值、整体 `degraded`；FY2025四问API曾因响应模型漏 `metric_code` 返回500，已在工作树修复并通过API回归测试，待干净 SHA 验收。诊断 API 哈希和初始工作树指纹见[Gate 1 证据记录](../60_delivery/verification/2026-09-26--damai-visibility-gate1-v1.md)；因源工作树含未提交改动，该快照不等同干净 SHA 验收。详见[大麦数据后的剩余体验收口](../50_plans/work_items/UX--post-damai-experience-closeout.md)。合成数据不解除公开 C 级或真实企业门禁。
