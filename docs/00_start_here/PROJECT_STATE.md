...
截至 2026-09-26，远端 `main` 为 `face8279`，迁移头为 `0030_page_anchor_vocabulary`；最新 SHA 的 CI 需按同一 SHA 单独核验。U8、S01、前端一致性 Task 0–9、大麦数据包和常驻库 G2 已完成；测试库隔离已于 `b60c51b` 修复，新的小米 2026H1 与阿里 FY2027Q1 holdout 已冻结并录入独立 oracle（`926af825`）。公开 C 级交叉评的42项确认异常已在 `4111f6a` 判定为真实抽取错误；109项口径疑点待裁决，京东物流200格英文原件已于 `b161fff` 冻结、待重核，抽取修订和新留出盲测未完成。大麦数据合同有24个月经营明细（1,920实际、10,752预算、4,800应收回款、672财务实际），但本地只读 API 抽查显示仅两份已发布年度报表、FY2025/FY2026 指标覆盖22/40和25/40，驾驶舱为 `degraded`（月度趋势及毛利矩阵有未发布项）；数据可见性修复工作包已提出，尚待用户批准实施。后续主线只在 `main` 串行推进。
