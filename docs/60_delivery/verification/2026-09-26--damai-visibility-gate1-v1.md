...
4. FY2025 四问工作台 GET 返回500，FY2026 返回200。500 的根因尚未确定，应作为 API 运行时故障跟踪，不推断为“数据缺失”。
5. `/data` 页面初始状态没有批次历史 GET，因此已装载批次在页面上不可浏览；这是可发现性/页面入口缺口，不是事实不存在。
6. `/public` 与 `/internal` 是导航落地页，不是数据报表页；业务目标页应单独计入覆盖验收。

## Gate 1 状态

诊断范围的 API 读数与哈希已固定；完整 Gate 1 仍需完成逐项缺口归因、FY2025 500 根因确认以及干净提交 SHA 的复测。复测须覆盖目标页面的真实 GET；本记录只支持缺口定位与后续实施计划，不支持代码或产品验收完成声明。
