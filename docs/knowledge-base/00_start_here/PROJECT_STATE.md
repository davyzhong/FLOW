...
- 落地抓取流水线 `scripts/wechat_kb/`：文章发现（seed 队列 / 收件箱自动归类 / 微信公众平台接口 / RSS 四通道）、正文与图片抓取、Markdown 转换、财经相关性筛选、索引与 99_manifest 自动重建，出站请求带 host 白名单与内网地址防护；
- 首批 7 篇入库后，经用户提供的两个公众号合集（财务分析 84 篇、财务管理 138 篇）完成数据熊全量归档：新增 208 篇、0 失败 0 跳过，全部判 relevant，全库 218 篇（约 250MB，批量入库启用 PNG→JPEG 压缩）；
- 沉淀 `wechat-kb-sync` 技能与每周一 09:00 定时同步；通用引擎另沉淀为跨项目 `wechat-article-harvest` 技能；
- 全量历史回填的浏览器登录与接口验证已打通（fakeid 已沉淀进 sources.yaml），合集之外的散篇文章可按 wechat-kb-sync 技能第 4 节的慢节奏流程补齐（受公众平台 200013 频率限制约束）。
