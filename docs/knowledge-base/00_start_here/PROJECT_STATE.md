...
- 建立公众号素材库 `docs/knowledge-base/08_wechat_sources/`（数据熊、数研复盘狮两来源），与阶段 0 研究资料同源，作为下一阶段方向验证的外部参照；
- 落地抓取流水线 `scripts/wechat_kb/`：文章发现（seed 队列 / 微信公众平台接口 / RSS 三通道）、正文与图片抓取、Markdown 转换、财经相关性筛选、索引与 99_manifest 自动重建，出站请求带 host 白名单与内网地址防护；
- 首批种子文章 2 篇已入库（数据熊 2026-08 财务分析报告、数研复盘狮 8 月毛利润与净利润分析报告）；
- 沉淀 `wechat-kb-sync` 技能并创建每周一 09:00 定时同步；全量历史回填需用户提供公众平台凭据（work/wechat_kb/mp_credentials.json，不入 git）。
