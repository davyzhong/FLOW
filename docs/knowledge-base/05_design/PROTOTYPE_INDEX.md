...
> 2026-09-05 增补：四表一注的可视化与图形分析（D043）由 P5 报告视图 `docs/implementation/p5/sf_2026q1_report_view.html` 承载（真实抽取数据，构建期闭合校验），评审台总览提供入口链接。
...
| `metric-library-v0-review.html` | 指标库 v0 评审台（唯一可视化产物，字典浏览器已归并于此）：40 通用 + 15 物流指标卡片、勾稽关系、CAS↔IFRS 映射、会计基础数据；支持逐项 纳入/待定/剔除 并复制筛选结果、仅看 MPM 过滤、头部生成时间戳（由 `scripts/generate_metric_library_review.py` 从 v0 数据集 YAML 生成） | 评审中（D040 立项后的实施前确认） |
