...
| D053（三层两模块与执行顺序；取代 D045/D046，修订 D049/D050） | 唯一路线图（U8→边界/Facts V2/安全门禁→公开模块 C 级→内部工作台→四级验证）、模块边界、U9/U10 重新裁决 | 主导航、规格索引、工作包 depends_on、模块 workstream/backlog 不形成第二执行入口 |
| D054（AI 生产链与证据/计算/学习原则） | 报告生产链、Finding 认知状态、发布阻断/override、证据分层、确定性计算边界、候选假设池 | AI 发布门禁、角色权限矩阵、复算机制、模型调用审计、历史学习存储分层 |
