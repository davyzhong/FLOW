# FLOW 标准 Excel 数据契约 V1｜2026-09-07 适用性复核
...
适用性复核：2026-09-07，读取基线 `04ba4d7`；冻结的 `flow.excel.v1` 字段与 fixture 不因本次文档整理改变。公开财报事实另受[统一事实合同](../superpowers/specs/financial-facts-contract.md)约束。接入能力于 Phase 3 落地。标准解析器负责结构有效的 FLOW 工作簿；当前 Intake 已支持非标准表画像、确定性映射、人工修正、转换审计、警告确认和正式导入发布。AI 是受约束候选接口，不代表默认接入了外部模型。完整操作见[接入文档](../intake/flow-v1-intake.md)。
...
外部工作簿使用业务编码。导入 PostgreSQL 时，系统以固定命名空间生成确定性 UUID，并保留每条事实的 `import_version_id` 和 `source_record_id`。标准包往返保留稳定行级身份；完整 Intake 另保存字段级 SourceRecord 与版本化转换事件，可追溯原文件、工作表、行、列、原值和转换值。
...
当前参考批次两项差异均为 `0.0000`。Intake 运行时对账默认容差为 `0.01`，记录期望值、实际值、差异与判定；超差阻断发布。fixture 的零差异基准与运行时允许阈值应分别理解。
...
单位数量级、毛利率、回款率与账龄阈值仍是规格中的扩展方向，不能视为已完整实施的规则目录。

解析器实现结构与关系阻断；Intake 质量服务产生质量问题与对账结果。每条可确认警告通过具名操作者和非空原因留痕；阻断问题不能确认放行。
