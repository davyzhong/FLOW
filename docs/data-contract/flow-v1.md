...
核对基线：2026-09-04，代码 `c1a59d1`。标准解析器负责结构有效的 FLOW 工作簿；当前 Intake 已支持非标准表画像、确定性映射、人工修正、转换审计、警告确认和正式导入发布。AI 是受约束候选接口，不代表默认接入了外部模型。完整操作见[接入文档](../intake/flow-v1-intake.md)。
